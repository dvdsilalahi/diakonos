<?php

namespace App\Http\Controllers;

use App\Models\MdEventDuty;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Gate;
use Illuminate\Support\Facades\Validator;

class MdEventDutyController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        //
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create()
    {
        //
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            "title"    => "required|array",
            "title.*"  => "required|string|max:50",
            "description"    => "array",
            "description.*"  => "nullable|string|max:256",
            "id"    => "array",
            "id.*"  => "nullable|integer",
        ]);
        if($validator->passes()){
            $validatedData = $validator->getData();

            if(count($validatedData['title']) === count(array_unique(array_map("strtoupper",$validatedData['title'])))){
                if(count($validatedData['title'])>0){

                    $duties = MdEventDuty::all();
                    foreach($duties as $duty){
                        if(!in_array($duty->id, $validatedData['id'])){
                            MdEventDuty::destroy($duty->id);
                        }
                    }

                    for ($x = 0; $x < count($validatedData['title']); $x++)
                    {
                        if(array_key_exists($x, $validatedData['id'])){
                            $data['id'] = $validatedData['id'][$x];
                            $data['title'] = strtoupper($validatedData['title'][$x]);
                            $data['description'] = $validatedData['description'][$x];
                            if(!MdEventDuty::select('uuid')->where('id',$data['id'])->get()){
                                $data['uuid'] = sha1($data['title']);
                            }
                            MdEventDuty::where('id',$data['id'])->update($data);
                        }
                    }
                    for ($x = 0; $x < count($validatedData['title']); $x++)
                    {
                        if(!array_key_exists($x, $validatedData['id'])){
                            $data['title'] = strtoupper($validatedData['title'][$x]);
                            $data['description'] = $validatedData['description'][$x];
                            $data['uuid'] = sha1($data['title']);
                            MdEventDuty::create($data);
                        }
                    }

                }
                return true;
            } else {
                $ndup = count($validatedData['title']) - count(array_unique(array_map("strtoupper",$validatedData['title'])));
                return json_encode(['title'=>["There is $ndup duplicate(s) in the 'title' column"]]);
            }

        } else {
            return $validator->getMessageBag();
        }


        if(count($request->title)>0){
            for ($x = 0; $x < count($request->title); $x++)
            {
                $duty = new MdEventDuty();
                if(array_key_exists($x, $request->id)){
                    $duty->id = $request->id[$x];
                }
                $duty->title = strtoupper($request->title[$x]);
                $duty->description = $request->description[$x];
                $duty->save();
            }
        }
        return true;
    }

    /**
     * Display the specified resource.
     */
    public function show(MdEventDuty $mdEventDuty)
    {
        //
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(MdEventDuty $mdEventDuty)
    {
        //
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, MdEventDuty $mdEventDuty)
    {
        //
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(MdEventDuty $mdEventDuty)
    {
        //
    }

    public function table()
    {
        if (! Gate::allows('admin')) {
            abort(403, 'You are not authorized.');
        }
        return view('master-data.table-event-duty',[
            'event_duties' => MdEventDuty::orderBy('title', 'ASC')->get(),
        ]);
    }

    public function select()
    {
        if (! Gate::allows('admin')) {
            abort(403, 'You are not authorized.');
        }

        return view('master-data.select-event-duty',[
            'event_duties' => MdEventDuty::orderBy('title', 'ASC')->get(),
        ]);
    }

    public function selectMinisterDuty()
    {
        if (! Gate::allows('admin')) {
            abort(403, 'You are not authorized.');
        }

        return view('master-data.select-minister-duty',[
            'event_duties' => MdEventDuty::orderBy('title', 'ASC')->get(),
        ]);
    }

    public function selectCommunityDuty()
    {
        if (! Gate::allows('admin')) {
            abort(403, 'You are not authorized.');
        }

        return view('master-data.select-community-duty',[
            'event_duties' => MdEventDuty::orderBy('title', 'ASC')->get(),
        ]);
    }


}
